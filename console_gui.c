...
void print_table2() {
    ...
    for(i = 0; i < table_height(); i++) {
        for(j = 0; j < table_width(); j++) {
            ...
            if(is_flagged(j, i))
                printf("|%sF%s",KGRN,KNRM);
            else if(!is_uncovered(j, i))
                printf("|X");
            else if(num_mines(j, i) == 0) // clean area
                printf("|%s%ld%s",KCYN, num_mines(j, i),KNRM);
            else if(num_mines(j, i) == 1) // the number of near mine is 1
                printf("|%s%ld%s",KYEL, num_mines(j, i),KNRM);
            else if(num_mines(j, i) > 1) // the number of near mine is greater than 1
                printf("|%s%ld%s",KRED, num_mines(j, i),KNRM);
            else
    ...
    if(game_mode == MENU) {
        printf("f (put/remove Flag in cell), c (Check cell), s (Setting), n (New game), q (Exit game): ");
    } else if(game_mode == FLAG) {
